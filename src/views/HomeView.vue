        ...
        <div class="flex h-full min-h-0 flex-col ">
          <div ref="menu" class="scrollbar-trigger flex h-full w-full flex-1 items-start border-white/20">
            ...
            <nav ref="navEle" class="flex h-full flex-1 flex-col space-y-1 p-2">
                ...
                <div class="flex flex-col gap-2 text-gray-100 text-sm">
                  ...
                  <template v-for="conversation, cidx in conversations">
                    ...
                    <div v-if="conversation.editable"
                      ...
                      <input id="titleInput" v-model="convTitletmp" @blur="titleInputBlur(cidx, conversation)" type="text"
                        class="text-sm border-none bg-transparent p-0 m-0 w-full mr-0" autofocus="true">
              ...
              <a href="https://github.com/DcrisWu/absa-front" target="_blank"
                class="flex py-3 px-3 items-center gap-3 rounded-md hover:bg-gray-500/10 transition-colors duration-200 text-white cursor-pointer text-sm">
...
const changeTheme = async (newTheme) => {
  ...
  html.style["color-scheme"] = newTheme;
...
function refrechConversation() {
  conversation.value = JSON.parse(JSON.stringify(conversation.value));
}
function chatRepeat() {
...
function send() {
  ...
  convLoading.value = true;
  var chatMsg = chatMsg.value;
  chatMsg.value = chatMsg.value.trim().replace(/\n/g, "")
  chatMsg.value = ""
  ...
  var that = this;
  var source = source.value = new EventSource(`/api/chat/${cid.value}?prompt=${encodeURIComponent(chatMsg)}`);
...
function editTitle(idx, conv) {
  ...
  conv.editable = true;
  this.$set(conversations.value, idx, conv);
  setTimeout(() => {
